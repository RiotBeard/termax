...
export function LocalAgentNotificationsBridge() {
  ...
  useEffect(() => {
    ...
    if (status === "awaiting-approval") {
      fire("attention", "TerMax needs your approval", "Approve a tool to continue");
    } else if (status === "error") {
      fire("error", "TerMax run failed", error ?? undefined);
    } else if (status === "idle" && isBusy(was)) {
      fire("finished", "TerMax finished", "Your task is ready");
    }
  }, [status, error]);
